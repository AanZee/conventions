...
### Overrule :focus style with :focus:hover for Chrome

- When you use the :focus pseudo-class you can highlight items when they receive focus. This can be useful when the user tabs through a form for example. The problem with Chrome is that it will also do this when you click on a button. Highlighting the button when you reach it by using the tab button is right behaviour. Highlighting the button when you actually click on it is unwanted behaviour, it draws too much attention. That attention is not needed because the user clicked on the element. A workaround for this is to use the :focus:hover pseudo-class. Override the highlighted style in this class. Because you are hovering the element when you click on it, it will show the overridden style.
